 ...
 *
 * High-density particle system on canvas with a reactive
 * magnetic vortex effect around the mouse/touch position.
 */
export const ASMRStaticBackground = () => {
  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      update() {
        ...
        if (dist < MAGNETIC_RADIUS) {
          const linear = (MAGNETIC_RADIUS - dist) / MAGNETIC_RADIUS;
          const force = linear * linear;
    ...
    const handleTouchMove = (e: TouchEvent) => {
      if (e.touches[0]) {
        ...
        setCursorVars(e.touches[0].clientX, e.touches[0].clientY);
      }
    };
